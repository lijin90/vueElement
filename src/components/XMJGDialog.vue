    ...
    <div id="XMJGDialog">
        ...
        >
            ...
            >
                ...
                <el-row>
                    <el-col :span="24">
                        <el-form-item label="填报单位" prop="REPORTED_COMPANY">
...
<script>
    export default {
        ...
        created() {
            ...
            //获取单位列表
            var datas = JSON.stringify({
                "tableName": "FL_SYS_JYDWZD",
                "queryFilter": {
                    "mapperId": "getJYDW",
                    "filterString": {
                        "mapperId": "getJYDW"
                    },
                    "selectFields": "C_ZQCODE,C_ZQNAME"
                }
            })
            that.$axios({
                method: "post",
                ...
                if (res.data.result == "success") {
                    for (var i = 0; i < res.data.data.length; i++) {
                        res.data.data[i].originalObjects.label = res.data.data[i].originalObjects.C_ZQNAME
                        res.data.data[i].originalObjects.value = res.data.data[i].originalObjects.C_ZQCODE
                        delete res.data.data[i].originalObjects.C_ZQNAME
                        delete res.data.data[i].originalObjects.C_ZQCODE
